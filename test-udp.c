...
int mealyrecvto(int recvsock, char *b, int bsize, int to)
{
  ...
  while ((count < (bsize-2)) && (c != '\n') && (c != '\0')) {
    ...
    if ((num = recvfrom(recvsock, b, MAXDATASIZE, 0, &server, &sizeof(server)) == -1) {
...
void udp_service(int msock)
{
  char buf[MAXDATASIZE] = "";
  int numbytes;

  ////// Send
  strcpy(buf,"FM:00_TO:01_ NP:1_HC:1_NO:nnn_VL:_DATA:register_me");
